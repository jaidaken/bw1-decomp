...
__attribute__((forced_callee_saves("esi"), MOV32rr_REV, no_callee_saves, decompose_imul, lea_chain_global_fold))
void __fastcall SetUpPhysOb__5AbodeFP6PhysOb(struct Object* this, const void* edx, struct PhysOb* param_1)
{
    void* frag = *(void**)((char*)this + 0x90);
    if (frag) {
        *(uint32_t*)frag = 0;
    }

    typedef uint32_t (__attribute__((thiscall)) *vt_fn_t)(struct Object*, int);
    vt_fn_t fn = ((vt_fn_t*)(*(void**)this))[0x788 / 4];
    uint32_t idx = fn(this, 0);

    extern char __opaque_global_0x00cc63e0 asm("_data_var_0x3003e0");
    char* addr = &__opaque_global_0x00cc63e0 + idx * 24;

    extern void __attribute__((thiscall)) __opaque_fn1(struct PhysOb*, uint32_t, char*) asm("_jmp_addr_0x007fb810");
    __opaque_fn1(param_1, 0x44fa0000, addr);

    extern void __attribute__((thiscall)) __opaque_fn2(struct PhysOb*) asm("_jmp_addr_0x007fbac0");
    __opaque_fn2(param_1);
}
